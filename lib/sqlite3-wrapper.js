...
Sqlite3.prototype.update_current = function(dev, old, cb) {
    ...
    switch (type) {
        case 'temp':
            ...
            if ((dev.t !== old.t) || (dev.h !== old.h)) {
                values.prev_ts = old.ts;
                values.prev_t = old.t;
                values.prev_h = old.h;
            }
        ...
        case 'switch':
            ...
            if ((dev.status !== old.status) || (dev.level !== old.level)) {
                values.prev_ts = old.ts;
                values.prev_status = old.status;
                values.prev_level = old.level;
            }
    ...
    this.update_values_where_logic(values, where, 'AND', dev.proto, cb);
};

/* update current device info */
Sqlite3.prototype.update_values_where_logic = function(values, where, logic, proto, cb) {
    var memory = this.memory;
    var type = rfx.get_type(dev.proto);
    var set = [];
    var w = [];
...
Sqlite3.prototype.get_by_name_type = function(name, type, cb) {
    ...
    var qs;
    if (name && type) {
        qs = [ 'SELECT * FROM', type, 'WHERE name = ? AND enabled = 1;' ];
        memory.get(qs.join(' '), name, cb);
    } else if (type) {
        qs = [ 'SELECT * FROM', type, 'WHERE enabled = 1;' ];
        memory.all(qs.join(' '), cb);
    } else {
        qs = [ 'SELECT * FROM temp LEFT OUTER JOIN switch ON',
               'temp.name = ? OR switch.name = ?', qs ];
               memory.get(qs.join(' '), name, cb);
        //qs = [ 'SELECT * FROM', type, 'WHERE name = ? AND enabled = 1;' ];
    }
};

Sqlite3.prototype.get_fields_by_type = function(fields, type, cb) {
    var memory = this.memory;
    var qs = [ 'SELECT', fields.join(), 'FROM', type, 'WHERE enabled = 1;' ];
    memory.all(qs.join(' '), cb);
};
